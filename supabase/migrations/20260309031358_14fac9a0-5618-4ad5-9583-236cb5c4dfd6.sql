...
CREATE POLICY "Users can view chat on own tickets" ON public.chat_messages FOR SELECT USING (
...
CREATE POLICY "Users can send chat on own tickets" ON public.chat_messages FOR INSERT WITH CHECK (
  (EXISTS (SELECT 1 FROM tickets WHERE tickets.id = chat_messages.ticket_id AND tickets.user_id = auth.uid()))
  OR has_role(auth.uid(), 'admin'::app_role)
);
...
CREATE POLICY "Authenticated can read settings" ON public.credit_settings FOR SELECT USING (true);
...
-- FIX 2: Remove dangerous user credit transaction insert policy
...
-- FIX 3: Remove user ticket event insert (allows fabricating audit history)
...
CREATE POLICY "Users can view own reviews" ON public.ticket_reviews FOR SELECT TO authenticated USING (
...
-- CONTACT_SUBMISSIONS (keep public insert as intended)
