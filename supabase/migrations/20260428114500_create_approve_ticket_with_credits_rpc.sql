...
DROP FUNCTION IF EXISTS public.approve_ticket_with_credits(uuid, numeric, uuid, text);
...
BEGIN
  IF auth.uid() IS NULL OR auth.uid() <> p_user_id THEN
    RAISE EXCEPTION 'Unauthorized: can only approve with your own credits';
  END IF;
...
GRANT EXECUTE ON FUNCTION public.approve_ticket_with_credits(uuid, numeric, uuid, text) TO public;
