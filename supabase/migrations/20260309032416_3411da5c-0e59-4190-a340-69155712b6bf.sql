...
-- RESTRICTIVE requires ALL policies to pass; PERMISSIVE requires ANY.
-- ============================================================
...
CREATE POLICY "Users can view chat on own tickets" ON public.chat_messages FOR SELECT TO authenticated
  USING ((EXISTS (SELECT 1 FROM tickets WHERE tickets.id = chat_messages.ticket_id AND tickets.user_id = auth.uid())) OR has_role(auth.uid(), 'admin'::app_role));
CREATE POLICY "Users can send chat on own tickets" ON public.chat_messages FOR INSERT TO authenticated
  WITH CHECK ((user_id = auth.uid()) AND ((EXISTS (SELECT 1 FROM tickets WHERE tickets.id = chat_messages.ticket_id AND tickets.user_id = auth.uid())) OR has_role(auth.uid(), 'admin'::app_role)));
...
CREATE POLICY "Users can view events on own tickets" ON public.ticket_events FOR SELECT TO authenticated
  USING ((EXISTS (SELECT 1 FROM tickets WHERE tickets.id = ticket_events.ticket_id AND tickets.user_id = auth.uid())) OR has_role(auth.uid(), 'admin'::app_role));
CREATE POLICY "Admins can insert ticket events" ON public.ticket_events FOR INSERT TO authenticated
  WITH CHECK (has_role(auth.uid(), 'admin'::app_role));
CREATE POLICY "Users can insert events on own tickets" ON public.ticket_events FOR INSERT TO authenticated
  WITH CHECK ((changed_by = auth.uid()) AND (EXISTS (SELECT 1 FROM tickets WHERE tickets.id = ticket_events.ticket_id AND tickets.user_id = auth.uid())));
...
CREATE POLICY "Users can view updates on own tickets" ON public.ticket_updates FOR SELECT TO authenticated
  USING ((EXISTS (SELECT 1 FROM tickets WHERE tickets.id = ticket_updates.ticket_id AND tickets.user_id = auth.uid())) AND (is_internal = false));
CREATE POLICY "Admins can view all ticket updates" ON public.ticket_updates FOR SELECT TO authenticated
  USING (has_role(auth.uid(), 'admin'::app_role));
CREATE POLICY "Users can add updates to own tickets" ON public.ticket_updates FOR INSERT TO authenticated
  WITH CHECK (EXISTS (SELECT 1 FROM tickets WHERE tickets.id = ticket_updates.ticket_id AND tickets.user_id = auth.uid()));
CREATE POLICY "Admins can insert ticket updates" ON public.ticket_updates FOR INSERT TO authenticated
  WITH CHECK (has_role(auth.uid(), 'admin'::app_role));
...
CREATE POLICY "Anyone can read published extensions" ON public.extensions FOR SELECT USING (published = true);
...
CREATE POLICY "Anyone can read published news" ON public.news_items FOR SELECT USING (published = true);
...
-- Create atomic credit purchase function to fix race condition
