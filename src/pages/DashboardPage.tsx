    ...
    <ProtectedLayout>
      <div className="mb-6">
        <motion.h1 {...fadeIn(0)} className="text-2xl font-bold text-foreground">
          Welcome{isGuest ? ", Guest" : ` back, ${profile?.full_name?.split(" ")[0] || "there"}`}
        </motion.h1>
        <motion.p {...fadeIn(0.05)} className="text-muted-foreground mt-1">
          {isGuest
            ? "Explore our Salesforce services hub — sign in to submit requests and track progress."
            : "Your Salesforce services hub — submit requests, track progress, and explore resources."}
        </motion.p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        {/* Center column - Ticket Submission (hidden for guests) */}
        {!isGuest && (
        ...
        {/* Left column */}
        <div className={`space-y-5 lg:col-start-1 lg:row-start-1 ${isGuest ? "lg:col-span-1" : ""}`}>
        ...
        {/* Right column */}
        <div className={`space-y-5 ${isGuest ? "lg:col-start-2" : "lg:col-start-3"} lg:row-start-1`}>
          <motion.div {...fadeIn(0.15)}>
            <AiAssistantWidget />
          </motion.div>
          <motion.div {...fadeIn(0.2)}>
