...
import { Cloud, Mail, Lock, User, Building2 } from "lucide-react";
...
export default function AuthPage() {
  ...
  const handleSalesforceSignIn = async () => {
    // Salesforce sandbox OAuth - custom implementation via edge function
    toast.info("Salesforce OAuth will redirect to your sandbox login. Configure SALESFORCE_CLIENT_ID and SALESFORCE_CLIENT_SECRET in your environment.");
    const { error } = await supabase.auth.signInWithOAuth({
      provider: "google", // placeholder - Salesforce OAuth requires custom edge function
      options: { redirectTo: window.location.origin + "/dashboard" },
    });
    if (error) toast.error(error.message);
  };
