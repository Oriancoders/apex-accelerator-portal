...
const ALL_FILTER = { label: "All", icon: <FileText className="h-4 w-4" />, color: "text-foreground", bg: "bg-muted" };

function categoryMeta(cat: string) {
  return CATEGORY_META[cat] ?? { label: cat, icon: <FileText className="h-4 w-4" />, color: "text-foreground", bg: "bg-muted" };
}

// Renders plain text article content with basic markdown-like formatting
  ...
  return (
    <div className="prose prose-sm max-w-none text-foreground space-y-3">
      {lines.map((line, i) => {
        ...
        if (line.startsWith("### ")) return <h3 key={i} className="text-base font-semibold mt-4 mb-1 text-foreground">{line.slice(4)}</h3>;
    ...
    <ProtectedLayout>
      <div className="max-w-5xl mx-auto space-y-8">
        ...
        <div className="relative rounded-2xl overflow-hidden bg-gradient-to-br from-primary/10 via-primary/5 to-background border border-primary/15 px-6 py-10 sm:px-10">
          ...
          <div className="relative">
            ...
            <p className="text-muted-foreground text-sm sm:text-base max-w-xl mb-6">
              Best practices, how-to guides, architecture patterns, and expert articles to help you get the most out of Salesforce.
            </p>
            {/* Search */}
              ...
              {featuredArticles.map((a) => {
                ...
                return (
                  <button
                    ...
                    className="text-left p-4 rounded-2xl border border-border bg-card hover:shadow-md hover:border-primary/30 transition-all group"
                  >
                    <div className={`w-9 h-9 rounded-xl flex items-center justify-center mb-3 ${cm.bg} ${cm.color}`}>
                      {cm.icon}
                    </div>
                    <p className="text-sm font-semibold text-foreground leading-snug line-clamp-2 group-hover:text-primary transition-colors">{a.title}</p>
        ...
        {isLoading ? (
          ...
          <div>
            {search && (
              <p className="text-xs text-muted-foreground mb-3">{filtered.length} result{filtered.length !== 1 ? "s" : ""} for "<span className="font-medium text-foreground">{search}</span>"</p>
            )}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {filtered.map((a) => {
                const cm = categoryMeta(a.category);
                const preview = a.content.replace(/#+\s/g, "").slice(0, 120).trim();
                return (
                  <button
                    key={a.id}
                    onClick={() => setSelected(a)}
                    className="text-left p-5 rounded-2xl border border-border bg-card hover:shadow-md hover:border-primary/25 transition-all group"
                  >
